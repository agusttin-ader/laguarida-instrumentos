...
/** Postgres `text[]` o JSON; siempre devuelve string[] acotado. */
  ...
  if (typeof raw === 'string') {
    ...
    if (s.startsWith('[')) {
      try {
        const p = JSON.parse(s)
        if (Array.isArray(p)) {
          return p.map((x) => String(x).trim()).filter(Boolean).slice(0, 10)
        }
      } catch {
        return []
      }
...
export default function normalizeProduct(raw = {}) {
  ...
  return {
    ...
    image_url,
    description,
    highlights: normalizeHighlightsField(raw.highlights),
    price,
    currency,
    aggregate_rating,
    review_count,
