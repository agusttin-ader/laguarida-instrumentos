...
export default function CatalogHorizontalTabs({ label, ariaLabel, items, className = '' }) {
  return (
    <nav aria-label={ariaLabel || label} className={`catalog-mobile-tabs md:hidden ${className}`}>
      {label ? (
        <p className="catalog-mobile-tabs__kicker mb-2 px-4 text-[10px] font-semibold uppercase tracking-[0.18em] text-[var(--dark-muted)] max-md:px-[var(--mobile-gutter)]">
          {label}
        </p>
      ) : null}
      <div className="catalog-mobile-tabs__scroll flex overflow-x-auto border-b border-white/[0.08] px-4 pb-px max-md:px-[var(--mobile-gutter)] [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden snap-x snap-mandatory">
        {items.map((item) => {
