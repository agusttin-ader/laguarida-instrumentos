...
export function getHomeBrandCatalogLink(brand, filters = {}) {
  const home = HOME_BRANDS.find((b) => b.id === brand.id)
  const marca = home?.filterBrand || brand.filterBrand
  // Import lazy-safe: build query inline to avoid circular deps with catalogFilters
  const p = new URLSearchParams()
  if (marca) p.set('marca', marca)
  const tipo = String(filters.tipo || '').trim()
  const precioMin = String(filters.precioMin || '').trim()
  const precioMax = String(filters.precioMax || '').trim()
