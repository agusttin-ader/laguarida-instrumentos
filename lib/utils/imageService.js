...
import { getStaticVariantUrl, isLocalProductImagePath, getStaticVariantSrcSet } from './staticImageVariants'
...
export const DISPLAY_PRESETS = {
  ...
  card: { width: 480, quality: 62 },
  carousel: { width: 640, quality: 66 },
  /** Ficha producto */
  galleryMain: { width: 1400, quality: 74 },
  galleryThumb: { width: 480, quality: 62 },
  lightbox: { width: 1800, quality: 78 },
...
/**
 * srcset para imágenes locales con variante grande (PDP / editorial).
 */
export function forDisplaySrcSet(src, variant = 'galleryMain') {
  const r = resolve(src)
  if (!r || !isLocalProductImagePath(r)) return null
  return getStaticVariantSrcSet(r, variant)
}
